...
describe("bot", () => {
    ...
    test("propagates errors from the task handler", async () => {
        mockHandler.handle = jest.fn().mockRejectedValue(new Error("Database unavailable"));
        const mockCtx = createMockContext("Build API");
        await expect(bot.handleTelegramMessage(mockCtx)).rejects.toThrow("Database unavailable");
    });
